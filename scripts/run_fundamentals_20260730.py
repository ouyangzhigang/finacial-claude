#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Fundamentals analyst batch (20260730, 46-stock pass list - 红利银行/资源石油有色/军工):
financial health + valuation anchor + red flags.
Input: technical-liquidity.json data.pass (46 stocks) + sector-analyst.json (sector/source).
Data: tencent qt.gtimg.cn (PE/price/mktcap) + eastmoney MAINFINADATA (financials) + sina BS (goodwill/equity).
No MCP used (本机 SSL 全挂). Output flat schema: passed/downgraded/vetoed/all arrays.
Bank-aware: 银行股天然高资产负债率(~90%)+无传统毛利率,不触发debtRatio/grossMargin软警示。
"""
...
for s in PASS_STOCKS:
    ...
    STOCKS.append({'code': s['code'], 'name': s['name'], 'price': s['price'],
                   'mktcap': s.get('marketCapYi'), 'sector': sec_info.get('sector', ''),
                   'source': sec_info.get('source', '')})

# Bank detection: by name or code. Banks excluded from debt-ratio/gross-margin soft flags.
BANK_NAME_KW = ('银行',)
BANK_CODES = {
    '601398', '601288', '601939', '601988', '601328', '600036', '601166', '600919',  # 红利银行pass list
    '600000', '600015', '600016', '601009', '601818', '601998', '601916', '002142', '600926', '601128', '601169',
}

def is_bank(code, name):
    return code in BANK_CODES or any(k in (name or '') for k in BANK_NAME_KW)

def derive_role(sector, source):
    s = (sector or '')
    if '银行' in s or '红利' in s:
        return '红利银行/高股息防御'
    if '军工' in s or '航空' in s or '船舶' in s or '兵器' in s:
        return '军工/地缘避险'
    if '石油' in s or '石化' in s or '能源' in s:
        return '上游资源/石油石化'
    if '有色' in s or '铜' in s or '金' in s or '铝' in s or '稀土' in s or '钼' in s:
        return '上游资源/有色'
    if '煤' in s:
        return '上游资源/煤炭'
    if '电力' in s or '核电' in s:
        return '公用事业/电力'
    if '建筑' in s or '铁路' in s:
        return '基建/交运'
    if '电信' in s:
        return '公用事业/电信'
    return '其他'
...
def analyze(code, name, sector, source, tc, fin_list, gw_wan, eq_wan):
    ...
    tc_pb = tc.get('pb') if tc else None
    ...
    if annual_loss and pe is not None and pe > 200:
        flags.append({'flag': '年报亏损且PE>200', 'severity': 'hard',
    ...
    # SOFT (降权) — 银行豁免debtRatio/grossMargin
    if not bank and debt_ratio is not None and debt_ratio > 70:
        flags.append({'flag': '资产负债率>70%', 'severity': 'soft',
                      'threshold': '<70%', 'actual': f'{debt_ratio:.1f}%', 'action': '降权'})
    if cf2p is not None and not bank:
        if cf2p < 0:
    ...
    if ar2r is not None and not bank and ar2r > 0.4:
    ...
    if not bank and gross_margin is not None and gross_margin < 10:
    ...
    # valuation anchor (绝对PE分层, 历史分位不可得标缺失)
    if pe is None:
        val_v = '数据缺失'
    elif pe < 0:
        val_v = '亏损无法估值'
    ...
    ctx_parts = []
    if bank:
        ctx_parts.append('银行股(豁免资产负债率/毛利率软警示)')
    if entry_type:
...
summary_text = (
    f'基本面排雷完成: {len(STOCKS)}只(红利银行/资源石油有色/军工三顺风)→'
    f'通过{len(passed)}/降权{len(downgraded)}/剔除{len(vetoed)}/数据缺失{len(missing)}。'
)
if vetoed:
...
data_caveats = (
    '数据源: 腾讯qt.gtimg.cn(PE/价格/市值/PB)+东财datacenter MAINFINADATA'
    '(ROE/负债率/毛利率/增速/BPS/每股现金流)+新浪资产负债表页(商誉/净资产)。'
    'MCP(iFind/wind/akshare)全SSL挂未用。'
    'PE/PB历史5年分位不可得(需历史估值时序)标记数据缺失,估值锚以绝对PE分层代替。'
    '商誉从新浪HTML解析(单位万元,转亿元),部分个股可能缺失。'
    '年报口径优先(REPORT_DATE以-12-31结尾,2025年报),无则取最新一期(半年报/三季报)。'
    '银行股天然高资产负债率(~90%)+无传统毛利率,豁免debtRatio/grossMargin软警示。'
    '行情为2026-07-30盘后(klineDate已收盘),基本面为底线排雷,硬雷点一票否决不作调和。'
)
...
output = {
    ...
    'data': {
        ...
        'softThresholds': {
            ...
            'cashflow': '经营现金流/净利润<0.5 或 <0 → 降权(银行除外)',
            'arRevenue': '应收账款/收入>40% → 降权(银行除外)',
