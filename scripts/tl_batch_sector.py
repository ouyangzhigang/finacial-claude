#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Technical Liquidity Agent - 短周期选股 20260721 batch analysis.
Direct Tencent HTTP (no subprocess) - handles SSL + GBK internally.
"""
import urllib.request, json, gzip, ssl, re, sys, os, time

SA_PATH = r'E:\finacial-invest\data\runs\20260721_short-term-picks\sector-analyst.json'
OUT_PATH = r'E:\finacial-invest\data\runs\20260721_short-term-picks\technical-liquidity.json'
SUBNEW_CODES = {'301583', '688797'}
...
def main():
    ...
    # Fetch K-lines
    print("Fetching K-lines and computing momentum...", file=sys.stderr)
    mom_map = {}
    ...
    for c in candidates:
        ...
        if not qt:
            reject_list.append({'code': code, 'name': name, 'reason': '行情数据缺失', 'sector': c.get('sector', ''), 'price': c.get('price', 0), 'dayPct': 0})
            factors_list.append({'code': code, 'name': name, 'm5': None, 'm10': None, 'm20': None, 'momentumUniformity': '行情数据缺失', 'volumeHealth': None, 'momentumAccel': None, 'entryType': None, 'entryScore': 0, 'meanReversionSignal': None, 'pullbackDepth': None, 'pullbackVolume': None, 'exhaustionProb': None, 'breakout': 'N/A', 'aboveMA20': None, 'rsi': None, 'technicalLevel': '行情数据缺失-降级为快照粗判', 'momentumQualityScore': 0})
            continue
        ...
        if hard_reasons:
            ...
            # Still add factors for rejected stocks
            if mom:
                factors_list.append(build_factor(code, name, mom, qt))
            else:
                factors_list.append({'code': code, 'name': name, 'm5': None, 'm10': None, 'm20': None, 'momentumUniformity': 'K线数据缺失', 'volumeHealth': None, 'momentumAccel': None, 'entryType': None, 'entryScore': 0, 'meanReversionSignal': None, 'pullbackDepth': None, 'pullbackVolume': None, 'exhaustionProb': None, 'breakout': 'N/A', 'aboveMA20': None, 'rsi': None, 'technicalLevel': 'K线数据缺失-降级为快照粗判', 'momentumQualityScore': 0})
            continue
    ...
    envelope = {
        ...
        'keyFields': {
            ...
            'passed': len(pass_list), 'rejected': len(reject_list),
            'totalCandidates': tot,
            'dataSource': '腾讯qt.gtimg.cn(行情)+web.ifzq.gtimg.cn(K线),SSL自处理',
            'note': 'ifind/akshare/wind MCP全挂SSL,走腾讯HTTP兜底',
            'hardThresholds': '日均成交额>=1亿,换手率20日均1-7%,流通市值>=30亿,非一字涨停/封死跌停,非透支(>70%),1w<40元,量比0.5-5,非ST/次新',
        }
    }
    os.makedirs(os.path.dirname(OUT_PATH), exist_ok=True)
    with open(OUT_PATH, 'w', encoding='utf-8') as f:
        json.dump(envelope, f, ensure_ascii=False, indent=2)
    print(f'\nPASS: {len(pass_list)}  REJECT: {len(reject_list)}  OUTPUT: {OUT_PATH}', file=sys.stderr)
    print(json.dumps(envelope['keyFields'], ensure_ascii=False, indent=2))

def build_factor(code, name, mom, qt):
