#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""批量获取A股实时报价快照(腾讯qt.gtimg.cn HTTP通道)"""
...
for line in resp.strip().split(';'):
    ...
    try:
        ...
        out[code] = {
            ...
            'amount_yi': round(float(m[37]) / 1e8, 2) if m[37] and m[37] != '-' else 0,
            'turnover_pct': round(float(m[38]), 2) if m[38] and m[38] != '-' else 0,
            'float_mktcap_yi': round(float(m[44]) / 1e8, 2) if m[44] and m[44] != '-' else None,
            'total_mktcap_yi': round(float(m[45]) / 1e8, 2) if m[45] and m[45] != '-' else None,
