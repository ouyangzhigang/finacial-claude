export const meta = {
  ...
  description: 'A股短周期选股——宏观→候选→流动性→catalyst∥fundamentals→risk→对抗审查→governor。数据落盘不占上下文',
  phases: [
...
// 历史对照与共享数据从 args 传入(workflow 运行时无 Bash/Read/Write)
const history = args.history || ''
const sharedCtx = args.sharedCtx || ''
...
// ── Phase 1: 宏观定调 ──
phase('宏观定调')
const macro = await S('macro', () => agent(P('macro-strategist','未来2周"天时"五维定调,输出顺风方向2-3。','用工具链取宏观读数+政策节点+情绪,锁定顺风方向(政策周期+市场热度双确认)。', ctx+sharedCtx), {agentType:'macro-strategist',schema:RET,label:'macro',phase:'宏观定调'}))
ctx = ap(macro,'宏观') + sharedCtx
log('宏观: ' + (macro?.summary || '空'))

// ── Phase 2: 候选池 ──
phase('候选池')
const sector = await S('sector', () => agent(P('sector-analyst','在顺风方向内撒网,生成候选池>=30只。','用 ifind_search_stocks(NL选股,医药类返空则手动龙头)+ifind_sector_data+cn_fetch.py rank 多源汇总30-50只去重,标注来源。cn_fetch 不覆盖的用 web-scraping fetch.py。1w账户优先<40元。', ctx), {agentType:'sector-analyst',schema:RET,label:'sector',phase:'候选池'}))
ctx += ap(sector,'候选池')
log('候选: ' + (sector?.summary || '空'))

// ── Phase 3: 流动性过滤 ──
phase('流动性过滤')
const tech = await S('technical', () => agent(P('technical-liquidity','对全部候选批量做流动性硬门槛过滤+短线因子。','用 Read 读 '+sector.path+' 的 data.candidates;用 ifind_get_stock_summary 或 cn_fetch.py factors 批量算5/10/20日动量+MA20+量价突破+amt20;硬门槛(成交额>=1亿/换手1-7%/市值>=30亿/非ST/近5-10-20日>30%透支剔除)。返pass/reject/factors。', ctx), {agentType:'technical-liquidity',schema:RET,label:'technical',phase:'流动性过滤'}))
ctx += ap(tech,'流动性')
log('流动性: ' + (tech?.summary || '空'))

// ── Phase 4: 并行评分(catalyst ∥ fundamentals) ──
phase('并行评分')
const [cat, fund] = await parallel([
  () => S('catalyst', () => agent(P('catalyst-scanner','对过关票批量做催化兑现度+情绪+资金。','用 Read 读 '+tech.path+' 的 data.pass;用 ifind_search_news(必带time_start/end)+china-news get_stock_news 取催化+情绪+资金,判断兑现度。非结构化页面用 web-scraping fetch.py。返每只催化+整体情绪。', ctx), {agentType:'catalyst-scanner',schema:RET,label:'catalyst',phase:'并行评分'})),
  () => S('fundamentals', () => agent(P('fundamentals-analyst','对过关票批量做财务排雷+估值锚。','用 Read 读 '+tech.path+' 的 data.pass;用 ifind_get_stock_financials(年报日期优先,max5主体可分批)+ifind_get_stock_shareholders 批量排雷+估值分位。硬雷点一票否决。', ctx), {agentType:'fundamentals-analyst',schema:RET,label:'fundamentals',phase:'并行评分'})),
])
