...
const [macro, fund, tech, cat] = await parallel([
  () => S('macro', () => agent(P('macro-strategist','对 '+tk+'('+nm+') 所属行业做"天时"五维定调。','用工具链取该股所属行业宏观读数+政策节点+情绪,输出顺风方向2-3+占优风格。', sharedCtx), {agentType:'macro-strategist',schema:RET,label:'macro',phase:'独立分析'})),
  () => S('fund', () => agent(P('fundamentals-analyst',tk+'('+nm+') 财务画像+估值锚+排雷。','用 ifind_get_stock_financials(年报日期优先)+ifind_get_stock_summary+ifind_get_stock_shareholders 取财务+估值分位+排雷,硬雷点一票否决。', sharedCtx), {agentType:'fundamentals-analyst',schema:RET,label:'fundamentals',phase:'独立分析'})),
  () => S('tech', () => agent(P('technical-liquidity',tk+'('+nm+') 流动性硬门槛+短线因子+技术位。','用 ifind_get_stock_summary 取近1月日K算5/10/20日动量+MA20+量价突破;流动性硬门槛(成交额>=1亿/换手1-7%/市值>=30亿)不过关直接标注。盘中价须收盘复核。', sharedCtx), {agentType:'technical-liquidity',schema:RET,label:'technical',phase:'独立分析'})),
  () => S('cat', () => agent(P('catalyst-scanner',tk+'('+nm+') 催化日历+兑现度+情绪+资金。','用 ifind_search_news(必带time_start/end)+china-news get_stock_news 取催化+情绪+资金,判断兑现度(近5日涨>15%半兑现/>30%透支)。非结构化新闻/公告页用 web-scraping fetch.py 抓取。', sharedCtx), {agentType:'catalyst-scanner',schema:RET,label:'catalyst',phase:'独立分析'})),
])
