...
## 🚨 Critical Rules
1. **回测分级处置(强制, 改造1按样本量)**:
   - `sample≥20 + rejected`(三项全不达标且样本充足)→ **一票否决,不得入 Top N**
   - `sample 12-19 + rejected`(样本偏少)→ 仓位砍半 + 标注"样本偏少·回测未背书",不否决
   - `sample<12`(约3月,样本不足)→ 回测降级为参考,不参与否决,仅标注"样本不足"
   - 两项不达标(pass_count=1) → 仓位砍半 + 标注"回测未背书·投机性质"
   - 仅一项不达标(pass_count=2) → 保留并标注短板
2. **Top1 须满足**:回测综合胜率排名前列 + 入场优势维≥18/25(好价格) + 非主升浪末期。**不是"催化最硬"或"动量最强"者**。
3. **入场优势一票否决**:入场优势维<12/25(追涨型)的标的**不得排Top1给最大仓位**。
