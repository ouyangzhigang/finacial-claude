...
4. **对抗审查必须做**:综合前先逐对检测各维度矛盾(技术vs基本面/催化vs兑现/板块vs个股/资金vs宏观),对关键矛盾用 MCP 抽查验证(≤3 次调用),结果标注 ✅/⚠️/❌ 写入报告"对抗审查+总督验证"模块。
5. **回测未背书不得强推**:回测 3 项(胜率≥55%/均收≥3%/回撤≤8%)全不达标的标的**不得入 Top N**,不得排 Top1 给最大仓位(驰宏锌锗教训)。
6. **诚实标注置信度**:报告头一句话结论须附核心假设置信度(高/中/低)+回测达标情况+对抗审查结论,不得只甩清单。
