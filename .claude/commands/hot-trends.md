...
- **回测验证(Step 7)必做**:最终推荐标的须回测近 3 月胜率/均收/回撤,三项全不达标剔除、两项不达标降仓;**跳过回测的推荐表视为未完成,不得落盘**;信心列填"高"须回测胜率≥55%+≥4维共振;
- **数据 soft-fail**:某 MCP 工具返回 error 时,按 §3 降级方案(换源或 curl 东方财富 push2 镜像/腾讯接口)继续,不中断整体分析。⚠️ **降级仍失败时(本机 push2his 常不通)标注"数据不可得",依赖该数据的推荐降级为"未验证",不得静默跳过继续推;资金流不得用别源估算(估算=编造)。**
