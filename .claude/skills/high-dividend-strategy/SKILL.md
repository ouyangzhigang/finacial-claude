...
### 第三点五步：硬性剔除闸门（一票否决,不进排名,须先于此步执行）

⚠️ 以下任一触发即**剔除出选股池,不得进入排名与前10名**,报告须单列"剔除名单"及触发原因。排名仅对通过全部闸门的标的计分——严禁"危险信号标注后仍凭股息率/总回报加权拿满分排Top1"（此为典型糊弄,等同驰宏案"回测不达标仍Top1"）：

1. 自由现金流覆盖 < 1.2 倍（现金分红超自由现金流,借钱分红）→ 剔除
2. 资产负债率 > 70%（非金融业）→ 剔除
3. 可持续性总分 < 50 → 剔除
4. 利润分红率 > 90% 且非银行/公用事业 → 剔除
5. 连续分红年数 < 3 → 剔除
6. 已公告当年取消/延期分红 → 剔除
