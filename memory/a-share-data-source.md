...
- **数据源优先级(2026-07-01 定型,2026-07-09 加 web-scraping + market_radar 层)**:① iFind MCP(财务/估值/股东/技术/新闻/指数/板块主力)→ ② cn_fetch.py(rank/factors/kline/squote 本地兜底,SSL 自处理)→ ③ web-scraping fetch.py(Scrapling,JS 渲染/反爬/非结构化页面,cn_fetch.py 不覆盖的端点)→ ④ curl -k 东方财富 datacenter(龙虎榜 RPT_DAILYBILLBOARD_DETAILS 等)→ ⑤ 标注"数据缺失"(北向实时本就无,2024-08 起停披露)。
- **market_radar.py 多源市场雷达(2026-07-09 新增)**:路径 `scripts/market_radar.py`;数据源=新浪(7x24快讯+榜单+日K)+腾讯(实时行情)+东方财富(板块+涨停池+龙虎榜+资金流);**核心信号自动提取**:🔴 critical(政策/黑天鹅)→ 🟡 important(板块异动≥3%/业绩超预期/大额资金/龙虎榜机构)→ 🟢 normal;信号类型覆盖 index/sector/sentiment/capital/news/policy 6 维。`--summary` 输出人类可读摘要;`--section` 只扫指定板块;`--ticker` 加个股维度。已集成到 prefetch_shared.py(自动调用 `--section index,sector,news,capital --summary` 写入 `_shared.json` 的 `radarSignals`)。
- **web-scraping fetch.py 集成(2026-07-09)**:路径 `.claude/skills/web-scraping/scripts/fetch.py`;auto 降级链 Fetcher(HTTP+TLS 伪装)→Dynamic(Playwright JS 渲染)→Stealthy(反爬/Cloudflare);`--no-verify` 默认(金融站点 SSL 常失败);`--json` 容错提取 API 响应(东方财富 502 带 body 可用);`--css` 选择器只取片段省 token。**定位:cn_fetch.py 与 curl 之间的兜底层**——cn_fetch.py 只覆盖 rank/factors/kline/quote/squote,非结构化页面/新闻/公告/JS 渲染页面用 fetch.py;curl 拿不到的反爬站点用 fetch.py。已写入 cheatsheet.md + 6 agent body + invest.md + hot-trends.md + CLAUDE.md。
